...
/*   Updated: 2020/11/05 10:27:50 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "FileReader.hpp"

FileReader::FileReader(char * const fileName, int bufferSize, char endLine):
_fileName(fileName),
...
FileReader::FileReader(FileReader const &src) {
	if (this != &src)
		*this = src;
	return ;
}
...
FileReader	&FileReader::operator=(FileReader const &src) {
	if (this != &src)
		*this = src;
	return *this;
}
...
const char		*FileReader::CannotOpenFileException::what() const throw() {
	return "Exception: could not open file";
}

tokenizedFile	FileReader::getFile() const {
	return this->_file;
}

void			FileReader::getFileOneLine(std::vector<std::string>	&line) const {
